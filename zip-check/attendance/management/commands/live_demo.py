...
class Command(BaseCommand):
    ...
    ("absent", 0.12),
    ("still_in", 0.09),
    ("late_arrival", 0.15),   # NEW: full day, but punched in late
    ("early_arrival", 0.05),  # NEW: full day, but punched in early
]
        ...
        def build_punch_pair(emp):
            ...
            if scenario == "full_day":
                ...
                punch_in = start + timedelta(minutes=random.randint(16, 60))
                punch_out = punch_in + timedelta(minutes=shift.full_day_minutes + random.randint(0, 20))
            elif scenario == "early_arrival":
                punch_in = start + timedelta(minutes=random.randint(-60, -1))
                punch_out = punch_in + timedelta(minutes=shift.full_day_minutes + random.randint(0, 20))
            else:  # still_in
