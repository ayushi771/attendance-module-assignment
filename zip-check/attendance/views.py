...
class AlertsView(generics.ListAPIView):
    """
    Returns TODAY's attendance records that need attention: currently
    late, currently early, or flagged as missing a punch-out. Scoped to
    today only -- an alert about something from days ago isn't
    actionable "live" information, it's historical noise.
    """
    serializer_class = AttendanceSerializer
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .filter(
                Q(is_late=True) | Q(is_early=True) | Q(missing_punchout_flagged=True),
